...
| `WebScrape` | prompt | scrape ผ่าน headless browser โดย HAL (รองรับหน้า JS, lazy load, ลบ selector ได้) — จะปรากฏเฉพาะเมื่อมี `HAL_API_KEY` |
...
pattern นี้ใช้ได้ทั่วไป tool ตัวใดก็สามารถประกาศ `requires_env`
แล้ว registry จะกรองออกเมื่อ env var ที่ระบุยังไม่ตั้ง HAL ทั้งสองตัว
